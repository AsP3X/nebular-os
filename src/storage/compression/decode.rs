...
// Human: Decode all blocks in order and concatenate into the logical payload.
// Agent: ITERATES layout.index; READS each block; APPENDS to output Vec.
pub fn decompress_blob(blob: &[u8], expected_size: u64) -> Result<Vec<u8>, StorageError> {
    if !super::format::is_compressed_blob(blob) {
...
// Human: Materialize logical bytes from a NOSB file to a spill path (fallback path for tooling).
...
pub fn decompress_file_to_temp(
    ...
    if read_header < FIXED_HEADER_LEN || !header.starts_with(super::format::BLOB_MAGIC) {
        std::fs::copy(blob_path, spill_path).map_err(|e| internal(anyhow::anyhow!(e)))?;
        return Ok(());
    }

    let layout = read_blob_layout(
        infile
            .try_clone()
            .map_err(|e| internal(anyhow::anyhow!(e)))?,
    )?;
    if layout.logical_size != logical_size {
        return Err(internal(anyhow::anyhow!(
            "blob header size mismatch: header={} metadata={logical_size}",
            layout.logical_size
        )));
    }
    ...
    for (idx, _entry) in layout.index.iter().enumerate() {
        let block_len = layout.logical_len(idx);
        let offset = layout.file_offset_for_block(idx);
        let block = read_block_payload(&mut infile, offset, block_len)?;
        out.write_all(&block)
            .map_err(|e| internal(anyhow::anyhow!(e)))?;
    }
...
mod tests {
    ...
    fn range_pump_returns_slice_without_full_file_decode() {
        ...
        .unwrap();
        assert!(super::super::format::is_compressed_blob(&blob));
