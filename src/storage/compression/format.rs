...
// Agent: BLOB_MAGIC="NOSB"; FIXED_HEADER_LEN=20; index follows header; block payloads after index.
pub const BLOB_MAGIC: &[u8; 4] = b"NOSB";
...
/// Returns true when `data` begins with the Nebular block-compressed header.
pub fn is_compressed_blob(data: &[u8]) -> bool {
    data.len() >= FIXED_HEADER_LEN && data.starts_with(BLOB_MAGIC)
}
...
pub fn parse_layout_bytes(data: &[u8]) -> Result<BlobLayout, StorageError> {
    if data.len() < FIXED_HEADER_LEN || !data.starts_with(BLOB_MAGIC) {
...
pub fn read_blob_layout(mut file: File) -> Result<BlobLayout, StorageError> {
    ...
    if !fixed.starts_with(BLOB_MAGIC) {
        return Err(internal(anyhow::anyhow!("not a block-compressed blob")));
    }
