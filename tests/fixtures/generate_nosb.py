"""Regenerate tests/fixtures/nosb with a build of commit da75495, the last one that wrote NOSB blobs
(the output is committed; see README.md).

Usage: python3 tests/fixtures/generate_nosb.py <scratch-dir> <da75495 nebular-os binary> tests/fixtures/nosb
...
SECRET = b"fixture-jwt-secret-0123456789abcdef-01234567"; PORT = 19096; BASE = f"http://127.0.0.1:{PORT}"
...
ENV = {"PATH": "/usr/bin:/bin", "HOME": os.environ["HOME"], "NOS_JWT_SECRET": SECRET.decode(), "NOS_BIND_ADDR": f"127.0.0.1:{PORT}",
       ...
       "NOS_COMPRESS_BLOCK_SIZE": "4096", "NOS_ZSTD_LEVEL_UPLOAD": "3"}
def start(**extra):
...
srv = start()
put("media", "docs/multi-block.txt", text(6000))
put("media", "docs/two-blocks.txt", text(750))
put("media", "docs/mixed.bin", text(3000) + random.randbytes(12000) + text(3000))
put("media", "docs/tiny.txt", b"below the compression threshold\n")
put("media", "raw/random.bin", random.randbytes(20000))
put("media", "video/clip.mp4", text(2000), ["Content-Type: video/mp4"])
put("media", "meta/typed.json", b'{"era": "nosb"}\n' * 400, ["Content-Type: application/json", "x-nd-custom-meta-origin: nosb-era"])
put("other", "docs/multi-block.txt", text(2500))
put("media", "trash/deleted.txt", text(900))
...
manifest = {
    "generated_by": "nebular-os at commit da75495 via tests/fixtures/generate_nosb.py",
